    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (data[i].rule == 1) {
                    tabel += "Admin"
                } else if (data[i].rule == 2) {
                    tabel += "Mahasiswa"
                } else {
                    tabel += "Karyawan"
                }
                tabel += "</td><td><a href='#' id='hapus" + data[i].id + "' onclick='tryHapus(" + data[i]
                    .id + ", \"" + data[i].nama + "\")' ><i class='fa fa-trash'></i></a></td></tr>"
...
function tambah() {
    if ($("#nama").val() == "") {
        ...
        $.ajax({
            type: 'POST',
            data: 'nama=' + $("#nama").val() + '&password=' + $("#password").val() + '&jabatan=' + $("#jabatan")
                .val(),
