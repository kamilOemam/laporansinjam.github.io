        ...
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <form enctype="multipart/form-data">
                    ...
                    <input type="file" id="uploadFotoNasabah" class="form-control" name="uploadFotoNasabah"
...
function tampilkan() {
    var baris =
        '<table class="table table-bordered" id="tabelNasabah" width="100%" cellspacing="0"><thead><tr><th>NO</th><th>NIK</th><th>NAMA</th><th>ALAMAT</th><th>SALDO</th><th>STATUS</th><th>KELOLA</th></tr></thead><tbody>'
    $.ajax({
        type: 'POST',
        ...
        success: function(data) {
            if (data.length) {
                for (let i = data.length - 1; i >= 0; i--) {
                    baris += "<tr><td>" + (data.length - i) + "</td><td>" + data[i].nik + "</td><td>" +
                        data[i].nama + "</td><td>" + data[i].alamat +
                        "</td><td>" + data[i].saldo + "</td><td>"
                    if (data[i].status == 1) {
                        baris += "Aktif"
                    } else {
                        baris += "Tidak Aktif"
                    ...
                    baris +=
                        ...
                        data[i].nik + "\", \"" + data[i].nama + "\", \"" + data[i]
                        .alamat + "\", " + data[i].status +
                        ")'><i class='fa fa-edit'></i><i class='mdi mdi-food'></i></button><button href='#' class='btn btn-info btn-sm ml-2' onClick='tryUpload(\"" +
                        data[i].nik + "\", \"" + data[i].nama + "\" ,\"" + data[i].foto +
...
function tambahAtauEdit() {
    if ($("#nik").val() == "") {
        ...
        $.ajax({
            type: 'POST',
            data: 'nik=' + $("#nik").val() + '&nama=' + $("#nama").val() + '&telp=' + $("#telp").val() +
                '&alamat=' + $("#alamat").val() + '&status=' + $("#status").val() + '&nikLama=' + $("#nikLama")
                .val(),
...
function tryUpload(nik, nama, foto) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function(data) {
            $("#fotoNasabah").attr('src', '<?= base_url() . "/public/upload/" ?>' + data.foto + "?=" +
                new Date().getTime())
